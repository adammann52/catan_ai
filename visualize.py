...
from handler import *
import time
from modelPlay import Qplay

class Visualize:

    def __init__(self,ai = False,game = None):
        self.game = game
        self.freeze = False
    ...
    def setUp(self):
        ...
        def bots():
            self.ai = True
            players.append('Dibbuk')
            players.append('Golem')
        ...
        Button(root,text='Play Bots',command=bots).grid(row=2,
                                                        column=1,
                                                        sticky=W)
        Button(root,
                  text='Start Game',
        ...
        if self.ai:
            self.Qnetwork = Qplay('models/cnn_bigr_ba400')

    #for the ai
    def updateHouses(self):

        for key in self.settlements_to_vertices:

            if self.settlements_to_vertices[key].owner:
                c.itemconfigure(key,fill = self.settlements_to_vertices[key].owner.color)

    #for the ai
    def updateRoads(self):

        for key in self.roads_to_edges:

            if self.roads_to_edges[key].owner:
                c.itemconfigure(key,fill = self.roads_to_edges[key].owner.color)
    ...
    def playGame(self):
        ...
        def nextTurn():
            ...
            if self.game.round > 1:
                ...
                self.updateDevs()
                c.itemconfigure(turn_label,text = self.game.current_player.name+"'s Turn")
                self.game.availableMoves()
                self.updateHand()


            if self.game.dieRoll == 7:
                if self.game.current_player.name not in ['Dibbuk','Golem']:
                    Seven.rolled(self,self.game.players)

            self.updateHand()
            self.game.availableMoves()

            if self.game.current_player.name in ['Dibbuk','Golem']:
                simulation(1,1)
        ...
        def clickEnd(event,tag):
            if not self.freeze and self.rolled:
                ...
                self.updateHand()
                self.rolled = False
                if self.game.current_player.name in ['Golem','Dibbuk']:
                    nextTurn()
        ...
        def enterHouse(event,tag):
            index =  settlements_to_vertices[tag]
            if 'settlements' in self.game.moves and self.game.moves['settlements'][index[0]][index[1]] and\
                    not self.freeze and self.rolled:
        ...
        def enterCity(event,tag):
            index =  cities_to_vertices[tag]
            if 'cities' in self.game.moves and self.game.moves['cities'][index[0]][index[1]] and\
                    not self.freeze and self.rolled:
                c.itemconfigure(tag,fill='#D3D3D3')
        ...
        def enterRoad(event,tag):
            index = roads_to_edges[tag]
            if 'roads' in self.game.moves and index in self.game.moves['roads'] and not self.freeze:
                c.itemconfigure(tag,fill='#D3D3D3')
        ...
        def updateRoads():
            ...
            for key in roads_to_edges:
                pair = roads_to_edges[key]
                edge = self.game.board.edges[pair]
                if edge.owner:
                    c.itemconfigure(key,fill = edge.owner.color)
                    boughtRoads.add(key)

        def updateHouses():

            for key in settlements_to_vertices:

                i,q = settlements_to_vertices[key]
                vert =self.game.board.vertices[i][q]
                if vert.owner:
                    c.itemconfigure(key,fill = vert.owner.color)
                    boughtHomes.add(key)

        def updateCities():

            for key in cities_to_vertices:
                i,q = cities_to_vertices[key]
                vert =self.game.board.vertices[i][q]
                if vert.owner and vert.city:
                    c.itemconfigure(key,fill = vert.owner.color)
                    boughtCities.add(key)
        def updateKnights():

            for key,knight in zip(tile_to_knight.keys(),tile_to_knight.values()):
                if knight[1].blocked:
                    c.itemconfigure(knight[0],fill = 'black')
                else:
                    self.current_robber = key
                    c.itemconfigure(knight[0],fill = '')
        ...
        def enterSim(event,tag):

            c.itemconfigure(tag,fill = 'white')

        def leaveSim(event,tag):
            c.itemconfigure(tag,fill = 'green')


        def simulation(event,tag):
            ...
            if self.game.current_player.name in ['Dibbuk','Golem']:
                ...
                if self.game.dieRoll == 7:
                    dropCards(self.game)
                state = getState(self.game)
                actions = processActions(self.game,state)
                a,_,_,w = selectAction(self.game,actions,self.Qnetwork)
                if a == 'E':
                    if self.game.round > 1:
                        clickEnd(1,1)
                    else:
                        nextTurn()
                elif not w and self.rounds < 500:

                    self.c.after(100,simulation(1,2))


        simulation(1,1)
        simulation_box = c.create_rectangle(5,950,135,900,fill = 'green')
        tmp = c.create_text(70,925,fill='black',font = 'Times 25 bold',
                            text = 'SIMULATE')
        c.tag_bind(simulation_box,"<Button-1>", lambda event,
                   tag = simulation_box: simulation(event,tag))
        c.tag_bind(simulation_box,"<Enter>", lambda event,
                   tag = simulation_box: enterSim(event,tag))
        c.tag_bind(simulation_box,"<Leave>", lambda event,
                   tag = simulation_box: leaveSim(event,tag))
        c.tag_bind(tmp,"<Button-1>", lambda event,
                   tag = simulation_box: simulation(event,tag))
        c.tag_bind(tmp,"<Enter>", lambda event,
                   tag = simulation_box: enterSim(event,tag))
