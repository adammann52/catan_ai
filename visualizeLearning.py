...
class Visualize:
    ...
    def __init__(self,ai = False,game = None,network = None):
        self.game = game
        self.freeze = False
    ...
    def playGame(self):
        ...
        def updateKnights():
            ...
            for knight in tile_to_knight.values():
                if knight[1].blocked:
        ...
        def simulation(event,tag):
            ...
            action, _, _, win = selectAction(self.game,actions,self.network,0)
            if action == 'E':
                self.game.playerUpdate()
            if not win and self.rounds < 600:
                self.rounds += 1
